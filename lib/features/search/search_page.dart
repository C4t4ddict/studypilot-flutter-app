...
class _SearchPageState extends State<SearchPage> {
  ...
  Widget build(BuildContext context) {
    final hasTyped = _lastQuery.trim().isNotEmpty;
          ...
          children: [
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('자료 탐색', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: AppColors.deepBlue)),
                  const SizedBox(height: 8),
                  const Text('검색 스테이션', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: AppColors.lightText)),
            ...
            if (_error != null)
              Padding(
                ...
                child: Container(
                  ...
                  child: Text('검색 중 오류가 발생했어: $_error', style: const TextStyle(color: Colors.redAccent)),
                ),
              ),
            const SizedBox(height: 14),
            Expanded(
              child: _results.isEmpty
                  ? Container(
                      ...
                      child: Center(
                        child: Text(
                          hasTyped ? '"$_lastQuery"에 대한 검색 결과가 아직 없어.' : '검색어를 입력하면 관련 학습 자료를 여기서 보여줄게.',
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return InkWell(
                          onTap: () => context.go('/search/${Uri.encodeComponent(item.id)}'),
                            ...
                            child: Row(
                              children: [
                                Container(
                                  ...
                                  child: const Icon(Icons.travel_explore_rounded, color: AppColors.primaryStrong),
                                ),
                                ...
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Text(item.title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: AppColors.lightText)),
                                      const SizedBox(height: 6),
                                      Text(item.subtitle ?? '연결 가능한 학습 자료 항목', style: const TextStyle(fontSize: 13, color: AppColors.lightMuted)),
                                    ],
                                  ),
                                ),
